...
info: The guide for contributors (or curious developers) who want to run NodiaFlow locally (on laptop, PC...)
---
...
Before you can install and use NodiaFlow, make sure you install the following on your computer:
...
NodiaFlow requires a redis cache to provide the best performance
...
Use environment variables or `.env` files to configure your project. More info [here](https://nodiaflow.com/developers/section/self-hosting/setup)
...
To build NodiaFlow server and seed some data into your database, run the following command:
```bash
yarn
```
...
## Step 8: Use NodiaFlow

**Frontend**

NodiaFlow's frontend will be running at [http://localhost:3001](http://localhost:3001).
You can log in using the default demo account: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

- NodiaFlow's server will be up and running at [http://localhost:3000](http://localhost:3000)
