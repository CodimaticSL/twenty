...
If you have an existing NodiaFlow instance with production data, **DO NOT** follow these steps as they will permanently delete your database!

While installing NodiaFlow for the first time, you might want to change the default database password.
...
During NodiaFlow installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. At NodiaFlow, we noticed that some VScode extensions were taking a lot of RAM so we temporarily disable them.
...
- Open your NodiaFlow app, navigate to `/settings`, and enable the **Advanced** toggle at the bottom left of the screen.
- Create a new webhook.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in NodiaFlow. Set the **Filters** to `Companies` and `Created`.
<div style={{textAlign: 'center'}}>
...
If you're running NodiaFlow behind a reverse proxy and experiencing connection issues:
...
2. **Check Reverse Proxy Settings:**
    ...
    - Confirm that your reverse proxy is correctly forwarding requests to the NodiaFlow server.
    - Ensure headers like `X-Forwarded-For` and `X-Forwarded-Proto` are properly set.

3. **Restart Services:**

    After making changes, restart both the reverse proxy and NodiaFlow containers.
...
- Community Support:
    ...
    Reach out to the [NodiaFlow community](https://github.com/twentyhq/twenty/issues) or [support channels](https://discord.gg/cx5n4Jzs57) for assistance.
