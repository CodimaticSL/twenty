...
<Warning>
**Important note:**
- Relations between objects in NodiaFlow are "One to Many". This means each record of object A can be attached to several records of object B. But each record of object B can belong to only one record of object A.
...
Yes. You need to create a field that you define as unique in your data model that will contain the `id` from your other tool(s). Please note that the name `id` is protected as it is used for the NodiaFlow id.
