...
4. Authorize NodiaFlow to access your Gmail and Google Calendar
...
4. Authorize NodiaFlow to access your Outlook and Microsoft Calendar
5. Your emails and calendar events will start syncing automatically
...
<ArticleWarning>
Only true mailboxes can be connected (e.g., support@domain.com with its own inbox). Email aliases that forward to another mailbox cannot be connected to NodiaFlow.
</ArticleWarning>
...
Control which email folders sync with NodiaFlow:
...
**Note**: We don't provide a CC email address for selective syncing. Instead, use the Message Folder feature above to achieve the same level of control over which emails sync with NodiaFlow.
